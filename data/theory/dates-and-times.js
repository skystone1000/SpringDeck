            ...
            blocks: [
                {
                    ...
                    code: '// Immutable and thread-safe: a static field is correct here, which it\n// never was for SimpleDateFormat.\nprivate static final DateTimeFormatter WIRE = DateTimeFormatter.ISO_INSTANT;\n\nprivate static final DateTimeFormatter HUMAN =\n        DateTimeFormatter.ofPattern("d MMMM yyyy", Locale.UK);\n\n// The classic pattern bug: uppercase YYYY is WEEK-BASED year -- and\n// WHICH weeks those are is LOCALE-dependent, so the same pattern gives\n// different answers on two machines. For 1 Jan 2027:\nDateTimeFormatter.ofPattern("YYYY-MM-dd", Locale.UK);  // 2026-01-01\n//   ISO weeks: 1 Jan 2027 falls in the week that belongs to 2026\nDateTimeFormatter.ofPattern("YYYY-MM-dd", Locale.US);  // 2027-01-01\n//   US week rules put the same day in week 1 of 2027\nDateTimeFormatter.ofPattern("yyyy-MM-dd");             // 2027-01-01\n//   <-- what you meant, on every locale\n\n// And the other pair:\n//   MM = month, mm = minute\n//   DD = day OF YEAR, dd = day of month\n//   HH = 0-23, hh = 1-12 (needs an am/pm marker to be unambiguous)\n\n// Parsing is strict by default and throws, which is right:\nLocalDate.parse("2026-02-30");      // DateTimeParseException',
            ...
            blocks: [
                {
                    ...
                    output: {
                        ...
                        lines: [
                            ...
                            'getValidOffsets on an overlap -> [+01:00, Z]   two answers -- ZoneOffset.UTC prints as Z -- and the API silently takes the first',
