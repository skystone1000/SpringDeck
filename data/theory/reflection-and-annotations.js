            ...
            blocks: [
                {
                    ...
                    items: [
                        ...
                        { name: 'Read the constructor', html: '<p>For each candidate, find the constructor and its parameter types. Parameter <em>names</em> need <code>-parameters</code> at compile time; Spring Boot\'s Maven and Gradle plugins set it, which is what lets a parameter be matched to a bean <em>by name</em> when the type alone is ambiguous, and what <code>@ConfigurationProperties</code> constructor binding relies on. <code>@Value("${x}")</code> needs none of it — the annotation names the property itself.</p>' },
